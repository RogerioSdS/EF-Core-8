...
namespace FuscaFilmes.API.EndpointExtensions
{
    public static class EndpointDiretores
    {
        ...
        public static void DiretoresEndpoints(this IEndpointRouteBuilder app)
        {
            ...
            app.MapGet("/diretores/firstOrDefault/{name}", DiretoresHandlers.GetDiretorByName).WithOpenApi();

            app.MapGet("/diretores/where/{id}", DiretoresHandlers.GetDiretoresById).WithOpenApi();

            app.MapPost("/diretores/", DiretoresHandlers.AddDiretores).WithOpenApi();
